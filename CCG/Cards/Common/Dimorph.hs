...
module CCG.Cards.Common.Dimorph (
  ...
  -- | The 'lhs' and 'rhs' functions extract their respective terms from
  -- a "Mapping", such that
  --
  -- prop> (:<=>:) <$> lhs <*> rhs == id
  lhs, rhs,
...
-- | The 'consistent' checks for consistency among the terms on both
-- sides of a list of "Mapping"s, returning "False" if any two distinct mappings
-- (including duplicates) have an LHS or RHS term in common, and "True"
-- otherwise.
...
-- performs the conversion, 'survey' first ensures that the "Mapping"s
-- it is given are all consistent, i.e. that no terms appear on either
-- side more than once, and then calling 'mapping' after this check. If
-- any "Mapping"s are duplicated in a list, duplicates are removed after
